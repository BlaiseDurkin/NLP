...
class Sentence(Node):
    def __init__(self, subject, predicate, cp=None, sc=None, greeting=None, question=None):
    ...
    def __repr__(self):
        return (f"greeting={self.greeting}, " if self.greeting else "") +(f", question={self.question}" if self.question else "") +(f", CP={self.sc}" if self.sc else "")+f"S(subject={self.subject}, predicate={self.predicate}" + (f", CP={self.cp}" if self.cp else "") + ")"
...
class QP(Node):
    def __init__(self, question: str):
        self.qp = question
    def __repr__(self):
        return f"QP(question='{self.qp}')"


class GP(Node):
    def __init__(self, greeting: str, receiver: str):
        self.gp = greeting
        self.r = receiver

    def __repr__(self):
        return f"GP(greeting='{self.gp}' receiver='{self.r}')"
...
ADV = {"peacefully","quickly","slowly","quietly","loudly","well","badly","recently","already","soon","now","then","here","there", "that", "roughly"}
#verbs
MODAUX = {"do", "does", "did", "didnt","dont", "doesnt",  "can", "cant", "could", "couldnt", "should", "shouldnt", "would",
          "woudnt", "am", "was","were", "is", "are", "will", "have", "had", "has", "shall", "may", "might", "must", "ought", "wont", "mustnt", "havent", "hasnt", "hadnt"}
#adverbs
QWORDS = {"what", "why", "where", "when", "how", "which", "whose"}

GREETINGS = {"hi", "hello", "hey", "heya", "sup", "yo", "ay", "ahoy", "oi", "oy", "bonjour", "hola"}
...
class RDParser:
    ...
    def is_modaux(self, tok): return tok in MODAUX
    def is_qword(self, tok): return tok in QWORDS
    ...
    def parseS(self) -> Optional[Sentence]:
        if self.ts.eof():
            return None
        #TODO -- sentence is question
        g_phrase = None
        if self.ts.peek() in GREETINGS:
            g_phrase = self.parseGP()
        q_phrase = None
        if self.ts.peek() in QWORDS or self.ts.peek() in MODAUX:
            q_phrase = self.parseQP()
        sub_conj = None
        if self.ts.peek() in CONJ:
            sub_conj = CP(self.ts.next(), self.parseClause())
        subj = self.parseNP()
        ...
        if subj is None:
            # can't find NP — attempt to recover: if sentence starts with 'that'/'who' etc treat as AP/CP
            print('no subject')
            return sub_conj #todo fix this: return greeting and question
        ...
        if pred is None:
            #print('no pred')
            #return subj
            Sentence(subj, pred, cp, sub_conj, g_phrase, q_phrase)

        return Sentence(subj, pred, cp, sub_conj, g_phrase, q_phrase)

    def parseGP(self) -> Optional[QP]:
        greeting = None
        receiver = None
        if self.ts.peek() in GREETINGS:
            greeting = self.ts.next()
            while self.ts.peek() in GREETINGS:
                greeting = self.ts.next()
        nxt = self.ts.peek()
        if nxt not in QWORDS and nxt not in MODAUX and nxt not in CONJ: #parseNP
            receiver = self.parseNP()
        return GP(greeting, receiver)

    def parseQP(self) -> Optional[QP]:
        #if not(VP/NP) Q AUX | AUX VP/NP --> question
        nxt = self.ts.peek()
        q_word = None
        aux = None
        if nxt in QWORDS:
            q_word = self.ts.next()
            if self.ts.peek() in MODAUX:
                aux = self.ts.next()
                return QP(q_word+" "+ aux)
            else:
                return QP(q_word)
        if nxt in MODAUX:
            aux = self.ts.next()
            return QP(aux) #todo -- check for exceptions
        return None
    ...
    def parseNP(self) -> Optional[NP]:
        ...
        words = []
        while nxt and not (self.is_qword(nxt) or self.is_modaux(nxt) or self.is_prep(nxt) or self.is_conj(nxt) or self.is_verb(nxt) or self.is_adv(nxt) or nxt in {')',','}):
        ...
        if words:
            ...
            if noun in PREPS or noun in CONJ or self.is_aux(noun) or self.is_verb(noun) or noun in ADV:
                ...
                print('mistake:', noun)
                return None #todo --> fix to not return None
        ...
        if not words:
            if det in ["that", "this", "those"]:
                return NP(None, [], det, pp, conj_np)
            return None #TODO - fix tto not return None
    ...
    """ def parseCP(self):
        ...
        return CP(conj, s)
    """
    def parseCP(self): # conjunction + clause
        tok = self.ts.peek()
        if tok not in CONJ:
            return None
        conj = self.ts.next()
    ...
    def parseCNP(self): # conjunction + noun phrase
        conj = self.ts.next()
        np = self.parseNP()
        if np is None:
            return None
    ...
    def pretty_print(self, node=None, indent=0):
        ...
        if isinstance(node, Sentence):
            ...
            if node.greeting:
                print(pad + "  greeting:")
                self.pretty_print(node.greeting, indent+2)
            if node.question:
                print(pad + "  question:")
                self.pretty_print(node.question, indent+2)
            if node.sc:
                print(pad + "  subordinate conjunction:")
                self.pretty_print(node.sc, indent+2)
            if node.subject:
                print(pad + "  Subject:")
...
if __name__ == "__main__":
    ...
    examples = ["john is sad", "that is sad"]
    #"""
    examples = [
    ...
    #"""
    #TODO:  -- WARNING --
    # - all questions fail this parser
